...
parser = argparse.ArgumentParser(description='Aggregate patient harm data by brand and manufacturer')
parser.add_argument('--patient-problems', default='patient_problem_counts.xlsx', 
                    help='Path to patient problems Excel file')
parser.add_argument('--maude-csv', default='maude_AUG2024_JULY2025.csv',
                    help='Path to MAUDE CSV file')
args = parser.parse_args()

# Load patient problems from Excel file
excel_path = args.patient_problems
if not os.path.exists(excel_path):
    print(f"Error: Patient problems file not found: {excel_path}")
    sys.exit(1)

df_patient = pd.read_excel(excel_path)
patient_problems_set = set(df_patient['Patient Problem'].dropna().astype(str))

csv_path = args.maude_csv
if not os.path.exists(csv_path):
    print(f"Error: MAUDE CSV file not found: {csv_path}")
    sys.exit(1)
...
import json
            ...
            if patient_str:
                try:
                    patient_list = json.loads(patient_str)
